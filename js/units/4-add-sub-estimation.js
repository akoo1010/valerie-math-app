        ...
        return [
            // 1. Multi-digit addition (up to 6-digit)
            {
                ...
                generate(diff, modality) {
                    ...
                    if (modality === 'worked-example') {
                        ...
                        result.workedExample = `<div style="text-align:center"><p><strong>Example:</strong> ${Engine.Utils.fmt(weA)} + ${Engine.Utils.fmt(weB)} = ?</p><p>Ones: ${weA % 10} + ${weB % 10} = ${(weA % 10) + (weB % 10)}${(weA % 10) + (weB % 10) >= 10 ? ' (carry 1!)' : ''}</p><p>Tens: ${Math.floor((weA % 100)/10)} + ${Math.floor((weB % 100)/10)} = ${Math.floor((weA % 100)/10) + Math.floor((weB % 100)/10)}</p><p>Keep going column by column...</p><p>Answer: <strong>${Engine.Utils.fmt(weA + weB)}</strong> 🎵</p></div>`;
                    } else if (modality === 'visual') {
                        result.visual += `<div class="visual-scaffold" style="color:var(--dance-cyan);"><p>🎶 Break it down by place value:</p><div>Ones: ${a % 10} + ${b % 10} = ${(a % 10) + (b % 10)}</div><div>Tens: ${Math.floor((a % 100)/10)} + ${Math.floor((b % 100)/10)} = ${Math.floor((a % 100)/10) + Math.floor((b % 100)/10)}</div><div>Hundreds: ${Math.floor((a % 1000)/100)} + ${Math.floor((b % 1000)/100)} = ${Math.floor((a % 1000)/100) + Math.floor((b % 1000)/100)}</div><div><strong>Total: ${Engine.Utils.fmt(answer)}</strong> 💃</div></div>`;
                    }
            ...
            // 2. Multi-digit subtraction with regrouping
            {
                ...
                generate(diff, modality) {
                    ...
                    if (modality === 'worked-example') {
                        ...
                        result.visual += `<div class="visual-scaffold" style="color:var(--dance-purple); margin-top:8px;"><p>🕺 Break it down by place value:</p><div>Ones: ${a % 10} − ${b % 10}${(a % 10) < (b % 10) ? ' → borrow!' : ''}</div><div>Tens: ${Math.floor((a % 100)/10)} − ${Math.floor((b % 100)/10)}</div><div>Hundreds: ${Math.floor((a % 1000)/100)} − ${Math.floor((b % 1000)/100)}</div><div><strong>Difference: ${Engine.Utils.fmt(answer)}</strong> 💃</div></div>`;
                    }
                ...
                generate(diff, modality) {
                    ...
                    const result = {
                        ...
                        visual: `<div style="display:flex;gap:16px;justify-content:center;align-items:center;font-size:1.3rem;color:var(--dance-purple);">
                            <div style="text-align:center;"><span style="font-weight:700;">${Engine.Utils.fmt(a)}</span><br><span style="font-size:0.8rem;">≈ ${Engine.Utils.fmt(estA)}</span></div>
            ...
            // 4. Estimate differences by rounding
            {
                ...
                generate(diff, modality) {
                    ...
                    if (modality === 'worked-example') {
                        const weA = R(2000, 7000), weB = R(1000, 3000);
                        const weEstA = Math.round(weA / roundTo) * roundTo;
                        const weEstB = Math.round(weB / roundTo) * roundTo;
                        result.workedExample = `<div style="text-align:center"><p><strong>Example:</strong> Estimate ${Engine.Utils.fmt(weA)} − ${Engine.Utils.fmt(weB)}</p><p>${Engine.Utils.fmt(weA)} ≈ ${Engine.Utils.fmt(weEstA)}</p><p>${Engine.Utils.fmt(weB)} ≈ ${Engine.Utils.fmt(weEstB)}</p><p>${Engine.Utils.fmt(weEstA)} − ${Engine.Utils.fmt(weEstB)} = <strong>${Engine.Utils.fmt(weEstA - weEstB)}</strong> 💃</p></div>`;
                    } else if (modality === 'visual') {
            ...
            // 5. Addition/subtraction word problem (dance-themed)
            {
                ...
                generate(diff, modality) {
                    ...
                    const scenarios = isAdd ? [
                        (a, b) => `🪩 On Friday night, ${Engine.Utils.fmt(a)} people came to the dance party. On Saturday, ${Engine.Utils.fmt(b)} more joined! How many dancers total?`,
                        (a, b) => `🎤 A singer sold ${Engine.Utils.fmt(a)} tickets for the first show and ${Engine.Utils.fmt(b)} for the encore. How many tickets in all?`,
                        (a, b) => `💃 The dance studio had ${Engine.Utils.fmt(a)} students last year and enrolled ${Engine.Utils.fmt(b)} new dancers this year. How many total?`
                    ] : [
                        (a, b) => `🕺 There were ${Engine.Utils.fmt(a)} people at the dance-off. ${Engine.Utils.fmt(b)} left after the first round. How many stayed?`,
                        (a, b) => `🎵 A playlist had ${Engine.Utils.fmt(a)} songs. The DJ removed ${Engine.Utils.fmt(b)} slow songs. How many are left?`,
                        (a, b) => `🪩 The disco ball lit up ${Engine.Utils.fmt(a)} times. ${Engine.Utils.fmt(b)} were during warm-up. How many were during the show?`
                    ];
                    ...
                    const result = {
                        ...
                        misconceptionHints: {
                            'wrong-operation': isAdd
                                ? `The problem says "more" or "total" — that means add! Try ${Engine.Utils.fmt(a)} + ${Engine.Utils.fmt(b)}. 🎵`
                                : `The problem says "left" or "removed" — that means subtract! Try ${Engine.Utils.fmt(a)} − ${Engine.Utils.fmt(b)}. 💃`
                        }
                    };

                    if (modality === 'worked-example') {
                        const weA = R(100, 500), weB = R(50, 200);
                ...
                generate(diff, modality) {
                    ...
                    const result = {
                        ...
                        misconceptionHints: {
                            ...
                            'picked-total': `That's the total, not the missing number. Use inverse operations to find what's missing! 💃`,
                            'picked-start': `That's the starting number, not the missing one. Think about what operation undoes ${isAdd ? 'addition' : 'subtraction'}. 🎶`
                        }
                    };

                    if (modality === 'worked-example') {
                        const weA = R(100, 500), weMissing = R(50, 200);
                ...
                generate(diff, modality) {
                    ...
                    if (ops === 'add') {
                        ...
                        if (modality === 'worked-example') {
                            ...
                            result.visual += `<div class="visual-scaffold" style="color:var(--dance-cyan); margin-top:8px;"><p>🎶 Break it down by place value:</p><div>Ones: ${a % 10} + ${b % 10} = ${(a % 10) + (b % 10)}</div><div>Tens: ${Math.floor((a % 100)/10)} + ${Math.floor((b % 100)/10)} = ${Math.floor((a % 100)/10) + Math.floor((b % 100)/10)}</div><div>Hundreds: ${Math.floor((a % 1000)/100)} + ${Math.floor((b % 1000)/100)} = ${Math.floor((a % 1000)/100) + Math.floor((b % 1000)/100)}</div><div><strong>Total: ${Engine.Utils.fmt(answer)}</strong> 💃</div></div>`;
                        }
                    } else if (ops === 'sub') {
                        ...
                        if (modality === 'worked-example') {
                            ...
                            const weAns = isAdd ? weEstA + weEstB : Math.abs(weEstA - weEstB);
